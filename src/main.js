...
function renderizar() {
  ...
  for( const mes of ano.meses) {
    ...
    cor.style.height = (mes.totalizadorDoMes.saldo*100)/10000;
  ...
  for (const mes of ano.meses) {
    ...
    addElement(linhaJuros, 'th', formatarDinheiro(mes.totalizadorDoMes.juros));
    ...
    addElement(linhaRendimentos, 'th', formatarDinheiro(mes.totalizadorDoMes.rendimentos));
    tabelaLancamentos.appendChild(linhaRendimentos);
    const linhaSaldo = document.createElement('tr');
    addElement(linhaSaldo, 'th', 'Total');
    addElement(linhaSaldo, 'th', formatarDinheiro(mes.totalizadorDoMes.saldo));
...
function adicionarLancamento() {
  ...
  ano.adicionarLancamento(
      mes.value,
      new Lancamento(categoriaLancamento.value, tipoLancamento.value, parseFloat(valorLancamento.value))
  );
